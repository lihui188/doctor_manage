<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		...
		:style="'top:72upx;min-width:146upx;'" v-show="isShowList">
		...
		computed:{
			...
			innerList(){    // 转换列表的数据格式
				...
				orginArr.forEach((val, index)=>{
					let value = typeof val === 'object' && 'value' in val ? val.name : val.name;
...
<style scoped lang="less">
	...
	@padding-left: 24upx;           //两侧的边距
	@arrowWidth: 50upx;            //右边的小三角按钮区域的宽度
	...
	.show-box{
		...
		.right-arrow{
			...
			top: 0;
			align-items: center; color: '#999999'; height: 100%;
		...
		/****** 列表框部分样式 *****/
		.list-container{
			position: absolute; width: auto; left: 0; top: 50px;
			box-sizing: border-box; z-index: 100;white-space: nowrap;
